/*
  ...
  Just to make picovga compatible with VGA_t4 API
...
//TP - I think these are upside down for the Pico DEMO board - this is where I suspect the colour mis-match is happening?
...
{
  printf("VGA_t4.cpp: executing begin\n");
  ...
  //is this what creates the scrambled camo. effect output?

  // create a semaphore to be posted when audio init is complete
  sem_init(&core1_initted, 0, 1);

//TP - output debug info
  printf("VGA_t4.cpp: about to init the vmode\n");
  multicore_launch_core1(core1_func);
  vmode = Video(DEV_VGA, RES_QVGA);
  printf("VGA_t4.cpp: vmode = %d\n",vmode);
  
  printf("VGA_t4.cpp: now we call VgaInitReql() inside vga.cpp\n");
  VgaInitReql(vmode);
...
//void VGA_T4::writeScreen16(int width, int height, int stride, uint16_t *buf, unsigned short * palette) {
//    uint16_t *buffer=buf;
//    uint16_t *src;
//
//    int i,j,y=0;
...
////                *dst++= col;
//                uint16_t pix = *src++;
//                uint8_t val = VGA_RGB(R16(pix),G16(pix),B16(pix));
////                vga_pixel val = palette[conv_pix];
//                *dst++ = val;
//                *dst++ = val;
//            }
//            y++;
...
static void AUDIO_isr() {
  ...
  // Alin: this was s = s/2 + 32767 but it had aliasing so i removed it. it works ok now
  s = s/2;
