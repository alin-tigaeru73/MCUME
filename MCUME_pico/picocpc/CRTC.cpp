...
bool CRTC::isVSyncActive()
{
    ...
    auto charLinesCounted = _charLineCounter - _registers[7];
...
uint16_t CRTC::generateAddress()
{
    ...
    uint16_t currentMemoryAddr = _memoryStartAddress + (_registers[1] * _charLineCounter) + _horizontalCounter;

    uint16_t bits0To10 = (currentMemoryAddr & 0b0000001111111111) << 1;
    uint16_t bits11To13 = (_scanlineCounter & 0b0000000000000111) << 11;
...
void CRTC::step()
{
    if(_microsecondCounter == 3)
    {
        ...
        {
            _memoryStartAddress = ((uint16_t) _registers[12] << 8) | _registers[13];
        }
