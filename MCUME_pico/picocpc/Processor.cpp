...
//    if(_interruptCounter == 19) _bus->psgExecute();
    ...
    // sound interrupts are 100 times per second, which is 0.1 interrupts per millisecond.
    // every 10 interrupts - loop with 1ms
    if(_interruptCounter == 9) _bus->psgExecute();

    _interruptCounter = _interruptCounter++ % 10;
}
